...
# -- _REGISTRY: dictionary mapping (group, command name) to _RegisteredCommand instances
_REGISTRY: dict[tuple[str | None, str], _RegisteredCommand] = {}
...
def register(name: str, *, hidden: bool = False, group: str | None = None, panel: str | None = None) -> Callable:
    def decorator(func: Callable) -> Callable:
        _REGISTRY[(group, name)] = _RegisteredCommand(func=func, hidden=hidden, group=group, panel=panel)
        return func
    return decorator

# -- registered_commands: returns every registered command mapping name to its registration details
def registered_commands() -> dict[tuple[str | None, str], _RegisteredCommand]:
